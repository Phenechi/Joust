...
public class HunterController : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.tag == "Player" && this.gameObject.tag == "Enemy")
        {
            ...
            if (!inBetween)
            {
                rb.AddForce(((collision.transform.position + new Vector3(0, 0.5f, 0)) - this.transform.position).normalized * 1.5f, ForceMode2D.Impulse);
    ...
    {
        UIManager.instance.killedHunter();
    }

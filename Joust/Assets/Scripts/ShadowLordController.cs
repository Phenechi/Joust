...
public class ShadowLordController : MonoBehaviour
{
    Rigidbody2D rb;
    public float speed;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.tag == "Player" && this.gameObject.tag == "Enemy")
        {
            ...
            if (!inBetween)
            {
                rb.AddForce(((collision.transform.position + new Vector3(0, 0.5f, 0)) - this.transform.position).normalized * 2.5f, ForceMode2D.Impulse);
